...
@export var throw_heigth_strength : float = 100.0
@export var throw_starting_height : float = 49
@export var hurt_box_damage : int = 0
...
func _player_interact() -> void:
	...
	if picked_up == false:
		...
		PlayerManager.player.lifted_item.add_child( throwable )
...
func throw() -> void:
	...
	vertical_velocity = -throw_heigth_strength
