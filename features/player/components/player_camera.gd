...
@export var shake_max_offset : float = 5.0 # Max shake pixeles
@export var shake_decay : float = 1.0 # When shake stops
var shake_trauma : float = 0.0

func _ready() -> void:
	LevelManager.tile_map_bounds_changed.connect(update_limits)
	update_limits(LevelManager.current_tilemap_bounds)
	PlayerManager.camera_shook.connect( add_camara_shake )
