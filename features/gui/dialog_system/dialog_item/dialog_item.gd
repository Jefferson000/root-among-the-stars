@tool
@icon( "res://features/gui/dialog_system/icons/chat_bubble.svg" )
...
func _on_selection_changed() -> void:
	...
	if not sel.is_empty():
		if self == sel[0]:
			example_dialog = load("res://features/gui/dialog_system/dialog_system.tscn").instantiate() as DialogSystemNode
			if example_dialog == null:
				return
			self.add_child(  example_dialog )
			example_dialog.offset = get_paranet_global_position() + Vector2( 32, -200 )
			check_npc_data()
			_set_editor_display()

func get_paranet_global_position() -> Vector2:
	var p = self
	var _checking : bool = true
	while _checking:
		p = p.get_parent()
